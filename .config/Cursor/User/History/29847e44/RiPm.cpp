...
// Returns true on success, false on error
static bool forward_response(int backend_fd, int client_fd) {
    char buf[8192];
...
void handle_client(int client_fd) {
    ...
    bool success = forward_response(backend_sock, client_fd);
    
    // Clean up backend socket
    close(backend_sock);
    
    // Note: client_fd is closed by the threadpool after handle_client returns
    // If we needed to handle errors differently, we could close it here too
    if (!success) {
        // Response forwarding failed - client might have disconnected
        // Threadpool will close the fd, so we don't need to do anything
    }
}
