-- ============================================
-- NoteForge AI - Complete Database Schema
-- ============================================
...
-- Create user_settings table for custom settings
...
-- Note_tags policies (users can only manage tags on their own notes)
CREATE POLICY "Users can view note tags for own notes" ON public.note_tags FOR SELECT 
  USING (EXISTS (SELECT 1 FROM public.notes WHERE notes.id = note_tags.note_id AND notes.user_id = auth.uid()));
CREATE POLICY "Users can add tags to own notes" ON public.note_tags FOR INSERT 
  WITH CHECK (EXISTS (SELECT 1 FROM public.notes WHERE notes.id = note_tags.note_id AND notes.user_id = auth.uid()));
CREATE POLICY "Users can remove tags from own notes" ON public.note_tags FOR DELETE 
  USING (EXISTS (SELECT 1 FROM public.notes WHERE notes.id = note_tags.note_id AND notes.user_id = auth.uid()));
...
-- Create storage bucket for note attachments (if it doesn't exist)
