...
func buildNodeIDToGRPC(nodeID string, kvPort string, peers []string) map[string]string {
	...
	for _, peer := range peers {
		// Extract port from peer address (e.g., "localhost:8002" -> "8002")
		parts := strings.Split(peer, ":")
		if len(parts) == 2 {
			raftPortStr := parts[1]
			// Try to infer node ID and gRPC port
			// For now, we'll use a simple heuristic: assume sequential numbering
			// This is a limitation - in production, you'd pass node IDs explicitly
		}
	}
