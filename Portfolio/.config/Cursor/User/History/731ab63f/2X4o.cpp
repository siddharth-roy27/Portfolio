...
LoadBalancer::LoadBalancer(const std::string &conf_path) : idx(0) {
    ...
    // Initialize health status for all backends (atomic<bool> is not copyable, so resize first)
    health_status.resize(backends.size());
    for (size_t i = 0; i < health_status.size(); ++i) {
        health_status[i].store(true, std::memory_order_relaxed);
    }
