...
Backend LoadBalancer::get_next_healthy_backend(size_t max_attempts) {
    // Similar to get_next_backend but with explicit max_attempts for retry logic
    size_t start_idx = idx.fetch_add(1);
    size_t attempts = 0;
    const size_t actual_max = std::min(max_attempts, backends.size());
    
    std::lock_guard<std::mutex> lock(health_mutex);
    
    while (attempts < actual_max) {
        size_t i = (start_idx + attempts) % backends.size();
    ...
    // No healthy backends found - return first one anyway
    return backends[0];
}

void LoadBalancer::mark_backend_healthy(size_t index, const std::string& reason) {
    std::lock_guard<std::mutex> lock(health_mutex);
    if (index < health_status.size() && !health_status[index]) {
        // Only log if status changed from unhealthy to healthy
        health_status[index] = true;
        Backend b = backends[index];
        Logger::backend_health(b.host, b.port, true, reason);
    } else if (index < health_status.size()) {
        health_status[index] = true;
    }
}

void LoadBalancer::mark_backend_unhealthy(size_t index, const std::string& reason) {
    std::lock_guard<std::mutex> lock(health_mutex);
    if (index < health_status.size() && health_status[index]) {
        // Only log if status changed from healthy to unhealthy
        health_status[index] = false;
        Backend b = backends[index];
        Logger::backend_health(b.host, b.port, false, reason);
    } else if (index < health_status.size()) {
        health_status[index] = false;
    }
}
...
Backend LoadBalancer::get_backend(size_t index) const {
    if (index < backends.size()) {
        return backends[index];
    }
    return Backend();  // Return empty backend if index out of range
}
